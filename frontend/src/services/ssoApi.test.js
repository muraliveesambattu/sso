...
describe('endpoints and verbs', () => {
  beforeEach(() => globalThis.fetch.mockResolvedValue(jsonResponse({ success: true, data: {} })));
  ...
  test('getSsoConfig unwraps body.data', async () => {
    globalThis.fetch.mockResolvedValue(jsonResponse({ success: true, data: { integration: { company_id: 'c1' } } }));
  ...
  test('listRolesApi unwraps the roles array', async () => {
    globalThis.fetch.mockResolvedValue(jsonResponse({ success: true, data: [{ role_id: 'r1' }] }));
...
describe('retry and error handling', () => {
  ...
  test('does not retry a non-retryable 404', async () => {
    globalThis.fetch.mockResolvedValue(jsonResponse({ error: { message: 'Not found' } }, { ok: false, status: 404 }));
    await expect(listRolesApi()).rejects.toThrow('Not found');
    expect(globalThis.fetch).toHaveBeenCalledTimes(1);
  });

  test('gives up after MAX_RETRIES on persistent 500s', async () => {
    globalThis.fetch.mockResolvedValue(jsonResponse({ error: { message: 'boom' } }, { ok: false, status: 500 }));
  ...
  test('surfaces success:false even on HTTP 200', async () => {
    globalThis.fetch.mockResolvedValue(jsonResponse({ success: false, message: 'Domain already configured' }));
  ...
  test('tags an aborted request with code TIMEOUT', async () => {
    ...
    expect(globalThis.fetch).toHaveBeenCalledTimes(4); // TIMEOUT is retryable
  });
